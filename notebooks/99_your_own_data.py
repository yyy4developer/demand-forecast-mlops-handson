...
⑥ モデルを作る        {TABLE} で需要予測モデルを作って、Unity Catalog に登録してください。
                      ⚠️ 「去年と同じ」と比べた結果も見せてください。
...
# MAGIC | ⚠️ 症状 | ⭐ 言い方 |
# MAGIC |---|---|
# MAGIC | 列の意味を誤解された | 「まず各列が何を表しているか教えてください」から始める |
# MAGIC | 日本語の列名でエラー | 「日本語の列名はバッククォートで囲んでください」 |
# MAGIC | 年月が変換できない | 「年月は `2024年1月` のような形式です。月末日にしてください」 |
# MAGIC | 数量が文字列扱いになる | 「数量に桁区切りが入っています。カンマを除いて数値にしてください」 |
# MAGIC | 日本語が化ける | 「文字コードは Shift_JIS かもしれません」 |
# MAGIC | Genie の答えがおかしい | ⚠️ ③ を飛ばしていませんか |
# MAGIC | 月数が足りないと言われる | ⚠️ モデル作成には各系列 **24 か月以上**必要。⭐ 足りなければ ⑤ を試す |
# MAGIC
