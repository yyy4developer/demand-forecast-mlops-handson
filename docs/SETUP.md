...
⚠️⚠️ **参加者が読める場所に作ってください。** 個人フォルダ (`/Workspace/Users/<自分>/`) に
作ると参加者がノートブックを開けません。

1. 左メニュー **「ワークスペース」** → ⭐ **`/Workspace/Shared/` を開く**
2. 右上 **「作成」** → **「Git フォルダ」**
3. このリポジトリの URL を入れて作成
4. ⭐ 作った Git フォルダを右クリック → **「共有」** → 参加者グループに **`CAN RUN`**

> ⚠️⚠️ **なぜ共有が必須なのか（2 つあります）**
>
> | | 理由 |
> |---|---|
> | 1 | 参加者は `notebooks/00`〜`99` を**このフォルダから開いて実行**します |
> | 2 | ⭐ `_config` が読む **`.databricks/bundle/dev/variable-overrides.json`** がここにあります |
>
> ⚠️ 2 が読めないと、`databricks.yml` の**既定のカタログ名**が使われ、
> 管理者が別名のカタログを作っていた場合は `NO_SUCH_CATALOG_EXCEPTION` になります。
>
> ⭐ 保険として `_config` は「見本スキーマ `fc_sample` を持つカタログ」を自動で探しますが、
> ⚠️ **共有しておくのが本筋です。**

> 💡 **参加者に自分で Git フォルダを作らせる場合**は、`.databricks/` が Git 管理外で
> 存在しないため、⭐ **`databricks.yml` の `variables.catalog.default` を実際の名前に
> 直してコミットしてください**（そうしないと自動検出に頼ることになります）。
