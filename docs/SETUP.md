...
⭐ **管理者用です。参加者は当日それぞれ自分で作ります**（後述）。
⭐ リポジトリは public なので、⚠️ **認証情報は要りません。**
...
⭐ **データを持ってこない参加者が必ずいます。** リポジトリに練習用 CSV を入れてあります。

| | |
|---|---|
| ファイル | ⭐ `data/sample_own_data.csv`（2.6 KB / 4 品目 × 30 か月 = 120 行） |
| ⭐ 特徴 | 列名が日本語 / 年月が `2024年1月` 形式 / 数量に桁区切り `2,155` / ⭐ **出ない月がある品目つき** |
| 使い方 | ダウンロードして自分の Volume にアップロード → ⭐ `99` の既定設定でそのまま通ります |

⭐ **`99` の「実データでよくつまずくところ」を実際に踏む**ように作ってあるので、
⚠️ 変換処理が効いていることを参加者が体感できます。

> 💡 講師は当日、このファイルも配れるようにしておくと親切です（Slack など）。

---

## ⭐⭐ 当日、参加者に伝えること（これだけ）

⭐ **参加者にはカタログ名だけ伝えます。** チャットに貼るなり、画面に出すなりしてください。
...
② notebooks/_settings.py を開いて 1 行だけ書き換える
     CATALOG = "<講師が伝えるカタログ名>"

③ notebooks/00_setup_my_pipeline から順に実行
```

⭐ **書き換えるのは 1 行だけ**です。自分のスキーマ名は
ログインユーザー名から `fc_ws_<あなた>` が自動で作られるので入力不要です。

> ⚠️ **参加者が自分の Git フォルダを作る前提**にしています。
> ⭐ そのほうが `07` で自分のジョブを作るときにパスが自分のものになり、
> 他の参加者と衝突しません。
