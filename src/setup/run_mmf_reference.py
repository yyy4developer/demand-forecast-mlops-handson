...
# MAGIC #
# MAGIC > ⚠️⚠️ **これは代替手段です。** 当日の本筋は `notebooks/08_mmf_genie_code` の
# MAGIC > とおり **Genie Code に自然言語で頼む**形です。
# MAGIC >
# MAGIC > ⭐ このノートブックは、⚠️ **Genie Code が使えない場合**や、
# MAGIC > ⭐ **見本の `scm_*` テーブルを手早く作りたい場合**の逃げ道として置いてあります。
# MAGIC
...
# MAGIC
# MAGIC > ⭐ **サーバーレスで動きます。** 統計モデルだけなら GPU もクラスタも不要です
# MAGIC > （実測: 6 モデル × 29 系列 × 3 ウィンドウで約 6.5 分）。
